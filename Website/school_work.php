<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="./stylesheet.css">
    <title>Opleidingen & Werk - Portfolio Jack</title>
</head>
<body>
    <?php include "./general_site_files/navbar.html" ?>

    <header>
        <h1>Opleidingen & Werk</h1>
    </header>
    <main class="school_main">
        <section class="school_work">
            <h2>Werkervaring</h2>
            <h3>Mei 2024 - Heden</h3>
            <p>Tankstation werknemer - Shell</p>

            <h3>Augustus 2023 - Februari 2023</h3>
            <p>Backend stagiair - Level Level</p>
            <ul>
                <li>Wordpress met eigen plugins en thema</li>
                <li>PHP Object Georiënteerd Programmeren</li>
                <li>HTML 5 in TWIG templates</li>
                <li>API integraties</li>
            </ul>

            <h3>Zomer 2022</h3>
            <p>Horeca werknemer - Plaswijckpark</p>

            <h3>Januari 2019 - Maart 2020</h3>
            <p>Allround medewerker - New York Pizza</p>
        </section>

        <section class="school_diplomas">
            <h2>Scholen en opleidingen</h2>
            <h3>Augustus 2022 - Heden</h3>
            <p>2e jaars Software Development MBO 4 - Techniek College Rotterdam</p>

            <h3>Februari 2021 - Juni 2022</h3>
            <p>VMBO-TL versneld VAVO trajet - Yulius onderwijs i.s.m Curio Breda</p>
            <img src="./images/school-photo.jpg" alt="Picture of me making an exam in highschool.">
        </section>
    </main>

    <?php include "./general_site_files/footer.html" ?>
</body>
</html>