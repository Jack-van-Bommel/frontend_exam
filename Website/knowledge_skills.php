<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Kennis & Vaardigheden</title>
</head>
<body>
	<?php include "./general_site_files/navbar.html" ?>

	<h1>Kennis & Vaardigheden</h1>

	<main>
		<div class="skills_backend">
			<h2>Backend kennis</h2>
			<ul>
				<li><a href="https://www.php.net/">PHP</a>, zowel procedural als Object Georiënteerd Programmeren</li>
				<li>Ervaring met <a href="https://www.mysql.com/">MySQL</a> database beheer in <a href="https://www.phpmyadmin.net/">PHPMyAdmin</a></li>
				<li>Basis kennis van <a href="https://learn.microsoft.com/en-us/dotnet/csharp/">C#</a></li>
				<li>Ervaring met API integratie</li>
			</ul>
		</div>

		<div class="skills_frontend">
			<ul>
				<li>Ervaring met het gebruik van <a href="https://developer.mozilla.org/en-US/docs/Web/HTML">HTML 5</a> om webpagina's te maken</li>
				<li>Basis kennis van HTML 5 gebruik in <a href="https://twig.symfony.com/">TWIG Template Management</a></li>
				<li>Ervaring met <a href="https://developer.mozilla.org/en-US/docs/Web/CSS">CSS</a> en <a href="https://sass-lang.com/">SASS</a></li>
				<li><a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript">JavaScript</a> beginner kennis</li>
			</ul>
		</div>

		<div class="skills_version">
			<ul>
				<li>Ervaring in het gebruik van <a href="https://github.com/">Github</a> voor versie controle en samenwerking</li>
				<li>Project beheer bijhehouden met de <a href="https://www.scrum.org/resources/what-scrum-module">SCRUM</a> methode</li>
				<li>Ervaring met het bijhouden van diverse project documentatie zoals design documenten, project plannen en test cases</li>
			</ul>
		</div>

		<div class="skills_programs">
			<ul>
				<li><a href="https://code.visualstudio.com/">Visual Studio Code</a> met diverse plugins</li>
				<li><a href="https://visualstudio.microsoft.com/">Visual Studio</a> voor C# projecten</li>
				<li>Basis kennis van <a href="https://unity.com/">Unity</a> voor game development, in samenwerking met Visual Studio</li>
				<li><a href="https://staruml.io/">StarUML</a> voor Class Diagrammen en Use Case Diagrammen</li>
			</ul>
		</div>
	</main>

	<?php include "./general_site_files/footer.html" ?>
</body>
</html>